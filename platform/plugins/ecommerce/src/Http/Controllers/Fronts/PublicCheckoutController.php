...
class PublicCheckoutController
{
    ...
    //session()->flush();
        ...
        if ($request->has('shipping_amount')) {
            $sessionCheckoutData['shipping_amount'] = $request->input('shipping_amount');
        }
        if ($request->has('product_code')) {
            $sessionCheckoutData['product_code'] = $request->input('product_code');
        }
        if ($request->has('local_product_code')) {
            $sessionCheckoutData['local_product_code'] = $request->input('local_product_code');
        }
        if ($request->has('shipping_method')) {
            $sessionCheckoutData['shipping_method'] = $request->input('shipping_method');
        }
        if ($request->has('shipping_option')) {
            $sessionCheckoutData['shipping_option'] = $request->input('shipping_option');
        }
//dd($request->input('shipping_amount'), $sessionCheckoutData);
        ...
        // grams to kilograms
        $sessionCheckoutData['weight']=$weight/1000;
        $dimensions=getPackageDimensions($sessionCheckoutData['weight']);
        // mm to cm
        $sessionCheckoutData['height']=$dimensions[2]/10;
        $sessionCheckoutData['length']=$dimensions[0]/10;
        $sessionCheckoutData['width']=$dimensions[1]/10;
...
//dd($sessionCheckoutData, $request->all());

// \Log::info("Session: --- ".print_r($sessionCheckoutData, true));
        ...
        if (is_plugin_active('marketplace')) {
            ...
            $shippingAmount = $request->input('shipping_amount', Arr::get($sessionCheckoutData, 'shipping_amount'));
            // $shippingAmount = 0;
            ...
            if ($isAvailableShipping) {
                ...
                if(empty($defaultShippingOption) || $request->input('country')=='AE'){
                    $dbDefaultShippingOption= null;
                    $shippingAmount = null;
                    $dbDefaultShippingMethod = $this->shippingRepository->getFirstBy(['country'=>'AE'],[],[],['rules']);
            
                    if(!empty($dbDefaultShippingMethod)){  
                                                  
                        $dbDefaultShippingOption = $dbDefaultShippingMethod->rules[0]['id'];
                        $defaultShippingOption = $dbDefaultShippingOption;
                        $shippingAmount = $dbDefaultShippingMethod->rules[0]['price'];                        
                ...
                $shippingAmount = Arr::get($shipping, $defaultShippingMethod . '.' . $defaultShippingOption . '.price', $shippingAmount);
...
//dd($shipping,$sessionCheckoutData, $shippingAmount);
            if (session()->has('applied_coupon_code')) {
        ...
        // $sessionCheckoutData['shipping_amount']=isset($sessionCheckoutData['shipping_amount']) && $sessionCheckoutData['shipping_amount']!=""? $sessionCheckoutData['shipping_amount'] : 0;
        // $sessionCheckoutData['product_code']=isset($sessionCheckoutData['product_code'])? $sessionCheckoutData['product_code'] : null;
        // $sessionCheckoutData['local_product_code']=isset($sessionCheckoutData['local_product_code']) ? $sessionCheckoutData['local_product_code'] : null;
...
//dd($data);
    ...
    protected function processOrderData(string $token, array $sessionData, Request $request, bool $finished = false): array
    {
        ...
        if($request->has('country')){
            ...
            $sessionData['country'] = $request->input('country');
        }

        // \Log::info("processOrderData Session: --- ".print_r($sessionData, true));
        // \Log::info("processOrderData Request: --- ".print_r($request->all(), true));
...
//dd($addressData,$sessionData);
    ...
    public function postCheckout(
        ...
        $request->merge([
            ...
            'user_id' => $currentUserId,
            'shipping_method' => $isAvailableShipping ? $shippingMethodInput : 100,
            'shipping_option' => $isAvailableShipping ? $request->input('shipping_option') : 100,
        ...
        if ($isAvailableShipping) {
            app(ShipmentInterface::class)->createOrUpdate([
                ...
                'shipment_id' => $shippingData ? Arr::get($shippingMethod, 'shipment_id', '') : 100,
