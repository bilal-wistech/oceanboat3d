...
<?php
$apiData = json_decode(session('apiData'));
// dd($apiData);
?>
<div class="card">
            ...
            <div class="row">
                <div class="col-12 border-top pt-2">
                    ...
                    @if ($order->shipment)
                        ...
                        <p><span class="d-inline-block">{{ __('Shipping Status') }}</span>: <strong class="d-inline-block text-info">
                        @if($order->dhl_tracking_number==null && $order->dhl_dispatch_number==null)
                            {!! BaseHelper::clean($order->shipment->status->toHtml()) !!}
                        @else
                            {{$order->dhl_shipping_status}}
                        @endif
                        </strong></p>
                        @if($order->dhl_tracking_number!=null || $order->dhl_dispatch_number!=null)
                            <p><span class="d-inline-block">{{ __('Shipping Company Name') }}</span>: <strong class="d-inline-block">DHL Express</strong></p>
                        @endif
                        @if ($order->dhl_tracking_number)
                            <p><span class="d-inline-block">{{ __('Tracking ID') }}</span>: <strong class="d-inline-block">{{$order->dhl_tracking_number}}</strong></p>
                        @endif
                        @if(isset($apiData))
                        @foreach($apiData as $key=>$shipment)
                            @if($key == "events")
                                <p><strong class="d-inline-block">Tracking Events</strong></p>
                                @foreach($shipment as $event)
                                    <p><strong>{{$event->typeCode}}</strong>: <span class="d-inline-block">{{ $event->description }} ({{$event->date}})</span></p>
                                @endforeach
                            @endif
                        @endforeach
                        @endif
                    ...
                    <div class="mt-2 row">
                        ...
                        @endif
                        @if ($order->canBeCanceled() && $order->dhl_tracking_number==null && $order->dhl_dispatch_number==null)
                            <div class="col-auto">
                                <a href="{{ route('customer.orders.cancel', $order->id) }}"
                                    onclick="return confirm('{{ __('Are you sure?') }}')"
                                    class="btn btn-danger btn-sm ml-2">{{ __('Cancel order') }}</a>
                            </div>
                        @elseif($order->dhl_shipping_status!="Delivered")
                            <div class="col-auto">
                                <a href="{{ route('track.order', $order->id) }}"
                                    class="btn btn-primary btn-sm ml-2">Track Order</a>
                            </div>
                        @endif
