...
trait Imageable {
	...
	*/
	...
	{
		return $this->morphOne(Image::class, 'imageable')->where('featured','!=',1);
	}
	...
	*/
	...
	{
		return $this->morphOne(Image::class, 'imageable')->where('featured',1)->orderBy('id','desc');
	}
	...
	*/
	public function saveImage($image, $featured = null)
	{
		...
		$img = IImage::make($image->path());
		$img->resize(500, 500, function ($constraint) {
			$constraint->aspectRatio();
		})->save($path.$image_fileName);

		return $this->createImage($path, $image->getClientOriginalName(), $image->getClientOriginalExtension(), $image->getMaxFilesize(), $featured);
	}
