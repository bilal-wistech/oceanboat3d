...
public function created(PredefinedList $predefinedList)
{
  if ($predefinedList->parent_id==0) {
    ...
    $list_details->save();
    foreach (config('predefinedlists.categories') as $key => $value) {
      $category=new PredefinedList;
      $category->parent_id=$predefinedList->id;
      $category->ltitle=$value['ltitle'];
      $category->descp=$value['descp'];
